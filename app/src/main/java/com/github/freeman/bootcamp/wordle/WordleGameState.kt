...
import java.lang.Character.MIN_VALUE as nullChar
 ...
 class WordleGameState(
    private val current_line: Int,private val grid: Array<Array<Tile>>,
    private val wordOnly: Boolean, private var wordToGuess: String
) {

    constructor(wordOnly: Boolean) : this(0,
        Array(8) { Array(5) { Tile(nullChar, TileState.EMPTY) } }, wordOnly,getRandomWord()
    )
     ...
     fun submitWord( word: String ): WordleGameState{
        ...
        if(wordOnly){
         ...
         if(current_line+1<  grid.size) {
             return WordleGameState(current_line + 1, grid, wordOnly, wordToGuess)
         }else {
             return this;
         }
    }
    companion object {
         ...
         */
        private fun load(fileName: String): List<String> {
